...
To complete the third part of this question we will duplicate our code from above for the new data. 
```{r}
coin1_df |> 
    ggplot() + 
        geom_line(aes(x=i, y=Xbar_in)) 
...
Next, we repeated this experiment twice more; three times total. 
```{r}
Coin <- bind_rows(coin1_df, coin2_df, coin3_df)
Coin |> 
    ggplot() + 
...
```{r}
bigger_coin <- gen_coin_cm(1000000, 4)
...
Suprisngly the biggest sample has ~fully converged to the population mean. 
>>>>>>> e63d3004728fe6e25783b8fdbe6d98c0ddcbc9c6:773Assignment2_BLUETEAM.qmd
...
