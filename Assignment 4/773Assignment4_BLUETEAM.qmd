...
Next, we will construct a table with counts of the different chains, split by state. 

```{r}
ck_df |> group_by(state) |> 
  count(chain) |> 
  mutate(
...
NJ minium wage increased from \$4.61 to \$5.08. The PA minium wage (the control group), stayed relatively the same increases from \$4.63 to \$4.62. 
