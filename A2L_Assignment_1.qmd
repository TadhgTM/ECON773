...
most individuals are concentrated in the middle of the earnings distribution. There is only a slight
upward-right trend, indicating a weak positive relationship between initial and later earnings.
This suggests that while higher earnings at assignment are somewhat associated with higher
earnings in year 4, there is substantial variation in earnings growth across individuals.
...
groups and their fitted lines. This is again simialr to the code above, but adds colours to points by treatment group ('assignment') using 'factor' to treat assignment as a categorical variable rather than a numeric one. This ensures that each group gets a separate colour and that geom_smooth() draws separate regression lines with confidence bands for each group.
...
The control group’s line (denoted in orange) is relatively flat, suggesting that initial earnings have little effect on year-4 earnings for this group. In contrast, the treated group’s line (denoted in blue) is slightly steeper, indicating that higher initial earnings are associated with higher year-4 earnings, and that the treatment amplifies earnings growth. Overall, this suggests that the treatment has a larger positive impact on earnings, particularly for individuals who started with higher earnings, while individual variation remains substantial.
